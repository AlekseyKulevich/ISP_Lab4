...
def index(request):
    context = dict(user_status=request.user.username)
...
def tasks_page(request):
    ...
    if request.user.is_authenticated:
        tasks = Task.objects.filter(
            user_creator__username__exact=request.user
        ).order_by("finish")
        context["tasks"] = tasks
        logger.info(
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if UserProfile.objects.filter(id=friend_id).exists():
                UserProfile.objects.get(id=request.user.username).friends.add(
                    UserProfile.objects.get(id=friend_id)
                )
                UserProfile.objects.get(id=friend_id).friends.add(
                    UserProfile.objects.get(id=request.user.username)
                )
                logger.info(
                    "Добавлен друг {} пользователю {}.".format(
                        friend_id, request.user.username
                    )
                )
        friends = get_user_friends(request.user.username)
        return render(
            request,
            "taskManager/friends.html",
            {"friends": friends, "user_status": request.user.username},
        )
    else:

        logger.info(
            "Страница друзей пользователя не была загружена, так как пользователь не авторизован."
        )
        return redirect("sign_in")
...
def remove_friend(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            friend_id = request.POST["name"]
            if UserProfile.objects.filter(id=friend_id).exists():
                UserProfile.objects.get(id=request.user.username).friends.remove(
                    UserProfile.objects.get(id=friend_id)
                )
                UserProfile.objects.get(id=friend_id).friends.remove(
                    UserProfile.objects.get(id=request.user.username)
                )
                friends = get_user_friends(request.user.username)
                logger.info(
                    "У пользователя {} был удален друг {}.".format(
                        request.user.username, friend_id
                    )
                )
                return render(request, "taskManager/friends.html", {"friends": friends})
        else:
            friends = get_user_friends(request.user.username)
            return render(
                request, "taskManager/friend_remove.html", {"friends": friends}
            )
    else:
        logger.info(
            "Страница друзей пользователя не была загружена, так как пользователь не авторизован."
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                logger.info(
                    "У пользовател {} задача c id {} не изменена, так как форма переданы неверные данные.".format(
